...
def slice_attention(q, k, v):
    ...
    while True:
        try:
            ...
            if steps > 128:
                raise e
            logger.info(f"out of memory error, increasing steps and trying again {steps}")
...
def pytorch_attention(q, k, v):
    ...
    try:
        ...
        logger.info("scaled_dot_product_attention OOMed: switched to slice attention")
        out = slice_attention(q.view(B, -1, C), k.view(B, -1, C).transpose(1, 2), v.view(B, -1, C).transpose(1, 2)).reshape(B, C, H, W)
    return out
...
class AttnBlock(nn.Module):
    def __init__(self, in_channels):
        ...
        if model_management.xformers_enabled_vae():
            logger.info("Using xformers attention in VAE")
            self.optimized_attention = xformers_attention
        elif model_management.pytorch_attention_enabled():
            logger.info("Using pytorch attention in VAE")
            self.optimized_attention = pytorch_attention
        else:
            logger.info("Using split attention in VAE")
