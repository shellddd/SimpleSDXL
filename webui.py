...
import args_manager
import copy
#from launch import reset_env_args 
    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            with gr.Group():
                ...
                with gr.Accordion(label='Wildcards & Batch Prompts', visible=False, open=True) as prompt_wildcards:
                    wildcards_list = gr.Dataset(components=[prompt], label='Wildcards: [__color__:L3:4], take 3 phrases starting from the 4th in color in order. [__color__:3], take 3 randomly. [__color__], take 1 randomly.', samples=wildcards.get_wildcards_samples(), visible=True, samples_per_page=28)
                ...
                with gr.Accordion(label='Describe Image', visible=False, open=False) as image_describe:
                    with gr.Row():
                        with gr.Column():
        ...
        with gr.Column(scale=1, visible=modules.config.default_advanced_checkbox) as advanced_column:
            with gr.Tab(label='Setting'):
                ...
                with gr.Accordion("Metadata", open=True, visible=True) as metadata:
                    with gr.Column():
            ...
            prompt_panel_checkbox.change(lambda x: [gr.update(visible=x, open=x if x else True), gr.update(visible=x, open=False)], inputs=prompt_panel_checkbox, outputs=[prompt_wildcards, image_describe], queue=False, show_progress=False, _js=switch_js).then(lambda x,y: wildcards_array_show(y['wildcard_in_wildcards']) if x else wildcards_array_hidden, inputs=[prompt_panel_checkbox, state_topbar], outputs=wildcards_array, queue=False, show_progress=False)
        ...
        if args_manager.args.enable_auto_describe_image:
            ...
            describe_input_image.upload(trigger_auto_describe, inputs=[describe_methods, uov_input_image, prompt, describe_apply_styles],
                                   outputs=[prompt, style_selections], show_progress=True, queue=True) \
                .then(fn=style_sorter.sort_styles, inputs=style_selections, outputs=style_selections, queue=False, show_progress=False) \
                .then(lambda: None, _js='()=>{refresh_style_localization();}')
